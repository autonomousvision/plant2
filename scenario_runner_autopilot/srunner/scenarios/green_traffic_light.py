...
from srunner.scenariomanager.scenarioatomics.atomic_behaviors import TrafficLightFreezer, LogBehaviour
...
class PriorityAtJunction(BasicScenario):
    ...
    def _create_behavior(self):
        ...
        """
        sequence = py_trees.composites.Sequence("PriorityAtJunction")
        tl_freeze = py_trees.composites.Parallel(policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        tl_freeze.add_child(WaitEndIntersection(self.ego_vehicles[0], self._junction.id))
        tl_freeze.add_child(TrafficLightFreezer(self._tl_dict))
        sequence.add_child(LogBehaviour(self.config.name))
        sequence.add_child(tl_freeze)
        return sequence
