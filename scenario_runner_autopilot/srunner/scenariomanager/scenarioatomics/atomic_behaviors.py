...
class LogBehaviour(py_trees.behaviour.Behaviour):
    def __init__(self, name):
        super(LogBehaviour, self).__init__(name)
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        self.name = name

    def update(self):
        print("Scenario", self.name, "Frame:", GameTime.get_frame())
        CarlaDataProvider.last_scenario = self.name
