...
class CollisionTest(Criterion):
    ...
    def _count_collisions(self, event):     # pylint: disable=too-many-return-statements
        ...
        if CarlaDataProvider.get_velocity(self.actor) < self.EPSILON:
            print("SKIPPED COLLISION DUE TO EPSILON!!!")
            return
            ...
            if linear_speed is not None:
                if linear_speed < self._min_speed and self._time_last_valid_state:
                    if (GameTime.get_time() - self._time_last_valid_state) > self._max_time:
                        ...
                        event.set_message('Agent got blocked at (x={}, y={}, z={}). Last scenario: {}'.format(
...
class OnSidewalkTest(Criterion):
    ...
    def _set_event_message(self, event, location, distance):
        ...
        event.set_message(
            '{} for about {} meters, starting at (x={}, y={}, z={}). Last scenario: {}'.format(
                message_start,
                round(distance, 3),
                round(location.x, 3),
                round(location.y, 3),
...
class OutsideRouteLanesTest(Criterion):
    ...
    def _set_traffic_event(self):
        ...
        self._traffic_event.set_message(
            ...
            "({}% of the completed route). Last scenario: {}".format(
...
class WrongLaneTest(Criterion):
    ...
    def _set_event_message(self, event, location, distance, road_id, lane_id):
        ...
        event.set_message(
            ...
            "road_id={}, lane_id={}. Last scenario: {}".format(
                round(distance, 3),
                round(location.x, 3),
                round(location.y, 3),
                round(location.z, 3),
        ...
        if self._terminate_on_failure and (self.test_status == "FAILURE"):
            ...
            if off_route:
                ...
                route_deviation_event.set_message(
                    "Agent deviated from the route at (x={}, y={}, z={}). Last scenario: {}".format(
                        round(location.x, 3),
                        round(location.y, 3),
                        round(location.z, 3),
                        CarlaDataProvider.last_scenario))
                ...
                if tail_wp.road_id == wp.road_id and tail_wp.lane_id == wp.lane_id and ve_dir.dot(wp_dir) > 0:
                    ...
                    if self.is_vehicle_crossing_line((tail_close_pt, tail_far_pt), (lft_lane_wp, rgt_lane_wp)):
                        ...
                        red_light_event.set_message(
                            "Agent ran a red light {} at (x={}, y={}, z={}). Last scenario: {}".format(
                                traffic_light.id,
                                round(location.x, 3),
                                round(location.y, 3),
                                round(location.z, 3),
        ...
        if not self.is_actor_affected_by_stop(check_wps, self._target_stop_sign):
            if not self._stop_completed and self._last_failed_stop != self._target_stop_sign.id:
                ...
                running_stop_event.set_message(
                    "Agent ran a stop with id={} at (x={}, y={}, z={}). Last scenario: {}".format(
...
class MinimumSpeedRouteTest(Criterion):
    ...
    def _set_traffic_event(self):
        ...
        if checkpoint_value >= self.success_value:
            ...
            self._traffic_event.set_message(f"Average speed is {checkpoint_value}% of the surrounding traffic's one. Last scenario: {CarlaDataProvider.last_scenario}")
            self.events.append(self._traffic_event)
    ...
    def terminate(self, new_status):
        ...
        if not self._terminated:
            if self.test_status == "FAILURE":
                ...
                traffic_event.set_message("Agent failed to yield to an emergency vehicle at (x={}, y={}, z={}). Last scenario: {}".format(
...
class ScenarioTimeoutTest(Criterion):
    ...
    def terminate(self, new_status):
        ...
        if timeout:
            ...
            traffic_event.set_message("Agent timed out a scenario at (x={}, y={}, z={}). Timed out: {}. Last scenario: {}".format(
                        round(self.actor.get_location().x, 3),
                        round(self.actor.get_location().y, 3),
                        round(self.actor.get_location().z, 3),
                        self._scenario_name,
