...
class DataAgent(AutoPilot):
  ...
  def setup(self, path_to_conf_file, route_index=None, traffic_manager=None):
    ...
    if self.save_path is not None and self.datagen:
      ...
      (self.save_path / 'bev_no_car_semantics').mkdir()
      (self.save_path / 'bev_no_car_semantics_augmented').mkdir()
  ...
  def save_sensors(self, tick_data):
    ...
    cv2.imwrite(str(self.save_path / 'bev_no_car_semantics' / (f'{frame:04}.png')), tick_data['bev_semantics'])
    cv2.imwrite(str(self.save_path / 'bev_no_car_semantics_augmented' / (f'{frame:04}.png')),
  ...
  def in_range(self, ego_matrix, vehicle):
    vehicle_transform = vehicle.get_transform()
    vehicle_matrix = np.array(vehicle_transform.get_matrix())

    rel_pos = t_u.get_relative_transform(ego_matrix, vehicle_matrix)
    x, y, z = rel_pos

    # x_div = 2**2 if x > 0 else 1 # TODO param
    # # Max front range (x>0) is 100m, other sides 50m
    # return x**2/x_div + y**2 < 50**2 and abs(z) < 30 # TODO param

    # Double radius in front
    return abs(z) < self.config.bb_save_radius and \
           abs(y) < self.config.bb_save_radius and \
           x < 2*self.config.bb_save_radius and \
           x > -self.config.bb_save_radius

  def get_bounding_boxes(self, lidar=None):

    # Save all scenario-vehicles with their scenario name
    scenarios = CarlaDataProvider.active_scenarios
    scenario_vehicles = defaultdict(str)
    for name, values in scenarios:
        if values[0] is not None:
            scenario_vehicles[values[0].id] = name
    ...
    for vehicle in vehicle_list:
      ...
      if self.in_range(ego_matrix, vehicle):
        if vehicle.id != self._vehicle.id:
          ...
          try:
            light_state = int(vehicle.get_light_state())
          except:
            light_state = 0
          ...
          result = {
              ...
              "scenario": scenario_vehicles[vehicle.id],
          }
          results.append(result)
    ...
    for walker in walkers:
      ...
      if self.in_range(ego_matrix, walker):
    ...
    for static in static_list:
      ...
      if self.in_range(ego_matrix, static):
        ...
        result = {
            ...
            'scale': static.attributes['scale'] if 'scale' in static.attributes else None
        }

        if result["mesh_path"] is not None and "Car/" in result["mesh_path"]:
          result["class"] = "static_car"
